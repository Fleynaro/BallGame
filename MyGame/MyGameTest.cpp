...
HRESULT InitWindow(HINSTANCE hInstance, int nCmdShow)
{
	...
	g_hWnd = CreateWindow(L"Urok3WindowClass", L"Урок 1: Создание устройств Direct3D", WS_OVERLAPPEDWINDOW,
...
HRESULT InitGeometry()
{
	...
	obj.Load(L"R:\\Rockstar Games\\MULTIPLAYER Dev\\MyGame\\MyGame\\obj\\Moon.obj");
	

	D3D11_BUFFER_DESC bd;	// Структура, описывающая создаваемый буфер
	ZeroMemory(&bd, sizeof(bd));				// очищаем ее
	bd.Usage = D3D11_USAGE_DEFAULT;
	bd.ByteWidth = sizeof(SimpleVertex) * obj.vertices.size();	// размер буфера
	...
	indicesShownCount = bd.ByteWidth = sizeof(WORD) * obj.indices.size();	// 36 вершин для 12 треугольников (6 сторон)
	bd.BindFlags = D3D11_BIND_INDEX_BUFFER; // тип - буфер индексов
	bd.CPUAccessFlags = 0;
	InitData.pSysMem = obj.indices.data();				// указатель на наш массив индексов
	...
	hr = CreateWICTextureFromFile(g_pd3dDevice, L"R:\\Rockstar Games\\MULTIPLAYER Dev\\MyGame\\MyGame\\obj\\Textures\\Bump_2K.png", nullptr, &g_pTextureRV);
	if (FAILED(hr)) return hr;
...
void UpdateMatrix(UINT nLightIndex)
{
	...
	if (nLightIndex == MX_SETWORLD) {
		// Если рисуем центральный куб: его надо просто медленно вращать вокруг оси
		g_World = XMMatrixRotationAxis(XMVectorSet(0.0f, 1.0f, 0.0f, 0.0f), t);
		nLightIndex = 0;
	}
...
void Render()
{
	...
	g_pImmediateContext->DrawIndexed(indicesShownCount, 0, 0);
